<?php
     ...
     */
        ...
        if(isset($_GET['supprimer'])) {
            ...
            $req=$db->query('DELETE FROM portfolio WHERE id= '.$_GET['supprimer']);
            ...
            die();
                ...
                if(isset($_GET['delete_images'])){
                    $id = $db->quote($_GET['delete_images']);
                    $select = $db->query("SELECT name, work_id FROM images WHERE work_id=$id");
                    $image = $select->fetch();
                    $images=glob('../img/posts/' . pathinfo($image['name'], PATHINFO_FILENAME) . '.*');
                    if(is_array($images)){
                        foreach($images as $v){
                            unlink($v);
                        }
...
<div class="container">
    <div class="row">
        ...
        <div class="col s12">
            <div class="input-field file-field">
                <div class="btn col s12">
                    ...
                    <input type="text" class="file-path col s10" readonly value="<?=$post->image?>" />
        ...
        <br>
            ...
            <a href="?page=post_image&id=<?= $post->id ?>&supprimer=<?=$post->id?>&delete_images=<?= $post->id?>" class="btn btn-danger" id="delete" name="delete" onclick="return confirm('Voulez-vous supprimer l\'article ?') onclick="document.location.href= '?'">Supprimer</a>
        </form>
